# My GitHub Copilot demo constitution
...
- **Spec-driven development**: every change starts from written specifications (`specs/`, standards, templates) so implementation always traces back to intent. The Specification by Example mindset (concrete user-facing examples, living documentation, shared ownership) keeps specs and code evolving together.
...
| `standards/` | Language- or domain-specific expectations (PYTHON.md, TYPESCRIPT.md, SECURITY.md, TESTING.md). |
| `templates/` | Ready-to-use scaffolds for PRDs and AI Agents. |
| `specs-template/` | Full specification workspace skeleton; copy this entire folder into your repo (typically as `/specs/`) to bootstrap multi-file specs. |
...
2. **Create or update specs first**: copy `specs-template/` into your repo, then follow the spec-driven workflow (`docs/REQUIREMENTS.md`, `docs/DESIGN.md`, etc.) to keep stakeholder-approved examples alongside requirements.
